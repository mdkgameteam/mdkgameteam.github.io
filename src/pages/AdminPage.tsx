...
type Contact = { nombre: string; email: string; mensaje: string; createdAt?: string };
type Project = { id: number; title: string; image: string; tags: string[]; description?: string };

const ADMIN_PASSWORD = 'admin';

function AdminPage() {
  const [auth, setAuth] = useState<boolean>(() => sessionStorage.getItem('mdk_admin_auth') === 'true');
  const [passwordInput, setPasswordInput] = useState('');
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [activeSection, setActiveSection] = useState('dashboard');

  const [contacts, setContacts] = useState<Contact[]>([]);
  const [category, setCategory] = useState<{ title: string; description: string; subtitle: string; projects: Project[] } | null>(null);
  ...
  useEffect(() => {
    if (!auth) return;
    try {
      const raw = localStorage.getItem('mdk_contacts');
      if (raw) setContacts(JSON.parse(raw));
      const rawProj = localStorage.getItem('mdk_projects');
      if (rawProj) setCategory(JSON.parse(rawProj));
    } catch (err) {
      console.error(err);
    }
  }, [auth]);
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    const saved = localStorage.getItem('mdk_admin_pass') || ADMIN_PASSWORD;
    if (passwordInput === saved) {
      sessionStorage.setItem('mdk_admin_auth', 'true');
      setAuth(true);
      setPasswordInput('');
    } else {
  ...
  const handleLogout = () => {
    sessionStorage.removeEventListener('mdk_admin_auth');
    setAuth(false);
  ...
  const removeContact = (index: number) => {
    ...
    copy.splice(index, 1);
    setContacts(copy);
    localStorage.setItem('mdk_contacts', JSON.stringify(copy));
  };

  const handleProjectAdd = (e: React.FormEvent) => {
    e.preventDefault();
    if (!category) {
      alert('No hay categoría cargada.');
      return;
    }
    const nextId = Math.max(0, ...category.projects.map((p) => p.id)) + 1;
    ...
    const updated = { ...category, projects: [newProject, ...category.projects] };
    setCategory(updated);
    localStorage.setItem('mdk_projects', JSON.stringify(updated));
    window.dispatchEvent(new Event('mdk_projects_updated'));
    setForm({ title: '', image: '', tags: '', description: '' });
  };

  const deleteProject = (id: number) => {
    if (!category) return;
    const updated = { ...category, projects: category.projects.filter((p) => p.id !== id) };
    setCategory(updated);
    localStorage.setItem('mdk_projects', JSON.stringify(updated));
    window.dispatchEvent(new Event('mdk_projects_updated'));
  };
        ...
        <div className="w-full max-w-md">
          <div className="bg-slate-900/80 backdrop-blur-xl rounded-2xl p-8 border border-slate-700/50 shadow-2xl">
            ...
            <p className="text-xs text-gray-500 text-center mt-6">
              Contraseña por defecto: <span className="text-neon-cyan font-semibold">admin</span>
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 hover:border-neon-cyan transition-all">
                  ...
                  <p className="text-3xl font-bold text-white">{category?.projects.length || 0}</p>
                </div>
                <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 hover:border-neon-cyan transition-all">
                  ...
                  {filteredContacts.map((c, i) => (
                    <div key={i} className="bg-slate-800 border border-slate-700 rounded-xl p-6 hover:border-neon-cyan/50 transition-all">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <button
                          onClick={() => removeContact(contacts.indexOf(c))}
          ...
          {activeSection === 'proyectos' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <button className="flex items-center space-x-2 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-emerald-500 hover:to-green-500 text-white px-4 py-2 rounded-lg font-medium transition-all">
                  <Plus size={20} />
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 space-y-4">
                ...
                <form onSubmit={handleProjectAdd} className="space-y-4">
                  ...
                  <div className="flex justify-end">
                    <button type="submit" className="bg-gradient-to-r from-neon-cyan to-blue-500 hover:from-blue-500 hover:to-neon-cyan text-white px-6 py-2 rounded-lg font-medium transition-all">
                      Agregar Proyecto
                    </button>
                  </div>
                </form>
                    ...
                    <div key={p.id} className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden hover:border-neon-cyan/50 transition-all group">
                      ...
                      <div className="p-4">
                        ...
                        <button
                          ...
                          className="w-full flex items-center justify-center space-x-2 bg-red-600/20 hover:bg-red-600/40 text-red-400 py-2 rounded-lg transition-all"
                        >
                          <Trash2 size={16} />
