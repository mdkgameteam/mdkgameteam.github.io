...
function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // persist contact to localStorage so admin can view it
    try {
      const existing = JSON.parse(localStorage.getItem('mdk_contacts') || '[]');
      const entry = { ...formData, createdAt: new Date().toISOString() };
      existing.unshift(entry);
      localStorage.setItem('mdk_contacts', JSON.stringify(existing));
      // reset form
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-xl p-8 border border-slate-700/50 animate-slide-in-right hover-glow">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                className="w-full bg-gradient-to-r from-neon-pink to-neon-cyan hover:from-neon-cyan hover:to-neon-pink text-white font-semibold py-3 rounded-lg transition-all duration-300 hover-lift neon-glow-strong flex items-center justify-center space-x-2 group"
              >
